...
def _build_first_law_eqs(underlying, edge_basis):
    ...
    b = np.array(
        [data["external_current"] for node, data in underlying.nodes(data=True)]
    )
    return M, b
...
def _build_cycle_row(cycle, edge_basis):
    ...
    row = np.array(
        [
            res
            if edge in cycle_in_edges
            else -res
            if (edge[1], edge[0]) in cycle_in_edges
            else 0
            for edge, res in edge_basis
        ]
    )
...
def _build_second_law_eqs(underlying, edge_basis):
    ...
    M = np.array([_build_cycle_row(cycle, edge_basis) for cycle in cycle_basis])
